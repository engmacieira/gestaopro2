...
async def delete_anexo( 
    ...
    if not success and anexo_deletado is None: 
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Anexo não encontrado para exclusão."
        )
    elif not success and anexo_deletado is not None: 
         raise HTTPException(status_code=500, detail="Erro interno ao deletar registro do anexo.")
    elif success and anexo_deletado:
        try:
            file_path = os.path.join(UPLOAD_FOLDER, anexo_deletado.nome_seguro)
            ...
            logger.exception(f"Erro ao deletar arquivo físico {anexo_deletado.nome_seguro} para anexo ID {id} (registro do BD já removido): {e}")
            raise HTTPException(status_code=500, detail="Erro ao deletar o arquivo físico associado.")
        except psycopg2.IntegrityError: 
             raise HTTPException(
                 status_code=status.HTTP_409_CONFLICT,
                 detail="Não é possível excluir o registro do anexo (erro de integridade)."
             )
        except Exception as e: 
             logger.exception(f"Erro inesperado ao deletar anexo ID {id} por '{current_user.username}': {e}")
             raise HTTPException(status_code=500, detail="Erro interno do servidor.")

def _generate_secure_filename(original_filename: str, tipo_doc: str | None, entidade_id: int, tipo_entidade: str, identificador_entidade: str) -> tuple[str, str]:
    """Gera um nome de arquivo seguro e o caminho relativo."""
    ...
    identificador_limpo = ''.join(c for c in identificador_entidade if c.isalnum() or c == '-').upper()[:10]
    timestamp = int(datetime.now().timestamp())

    nome_arquivo_final = f"{timestamp}_{tipo_doc_limpo}_{identificador_limpo}{extensao}"

    subpasta = str(entidade_id) 
    if tipo_entidade == 'aocs':
        subpasta = os.path.join('aocs', str(entidade_id))
    elif tipo_entidade == 'contrato':
         subpasta = os.path.join('contratos', str(entidade_id)) 

    caminho_relativo = os.path.join(subpasta, nome_arquivo_final).replace("\\", "/") 
    caminho_completo_pasta = os.path.join(UPLOAD_FOLDER, subpasta)

    return nome_arquivo_final, caminho_relativo, caminho_completo_pasta
...
async def upload_anexo_file( 
    id_entidade: int = File(...),
    tipo_entidade: str = File(...), 
    tipo_documento: str | None = File(None),
    ...
    if tipo_entidade not in ['contrato', 'aocs']:
        raise HTTPException(status_code=400, detail="Tipo de entidade inválido. Use 'contrato' ou 'aocs'.")
    ...
    try:
        ...
        logger.warning(f"Entidade {tipo_entidade} ID {id_entidade} não encontrada para upload por '{current_user.username}': {e}")
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
         logger.exception(f"Erro ao buscar entidade {tipo_entidade} ID {id_entidade} para upload: {e}")
         raise HTTPException(status_code=500, detail="Erro ao verificar entidade associada.")

    try:
        nome_final, caminho_relativo_db, caminho_pasta_destino = _generate_secure_filename(
        ...
        raise HTTPException(status_code=500, detail="Erro ao preparar salvamento do arquivo.")
    ...
    try:
        ...
        logger.info(f"Usuário '{current_user.username}' fez upload do Anexo ID {novo_anexo.id} ('{novo_anexo.nome_original}')")
        return novo_anexo
    except Exception as e:
        logger.exception(f"Erro ao criar registro no BD para anexo '{caminho_completo_arquivo}': {e}")
        if os.path.exists(caminho_completo_arquivo):
             try: os.remove(caminho_completo_arquivo)
             except OSError: logger.error(f"Não foi possível remover arquivo órfão: {caminho_completo_arquivo}")
        raise HTTPException(status_code=500, detail="Erro ao registrar o anexo no banco de dados.")
...
async def download_anexo_file(
    ...
    """Baixa o arquivo físico de um anexo."""
    repo = AnexoRepository(db_conn)
    anexo = repo.get_by_id(id)
    if not anexo:
        raise HTTPException(status_code=404, detail="Anexo não encontrado.")

    file_path = os.path.join(UPLOAD_FOLDER, anexo.nome_seguro)
    ...
    logger.info(f"Usuário '{current_user.username}' baixou Anexo ID {id} ('{anexo.nome_original}')")

...
from app.schemas.anexos_schema import AnexosRequest, AnexosResponse
from app.repositories.anexos_repository import AnexosRepository
...
@router.post("/", 
             response_model=AnexosResponse, 
             status_code=status.HTTP_201_CREATED)
def create_anexos(
    anexos_req: AnexosRequest, 
    db_conn: connection = Depends(get_db)
):
    try:
        repo = AnexosRepository(db_conn)
...
@router.get("/", response_model=list[AnexosResponse])
def get_all_anexos(
    db_conn: connection = Depends(get_db)
):
    repo = AnexosRepository(db_conn)
...
@router.get("/{id}", response_model=AnexosResponse)
...
@router.put("/{id}", response_model=AnexosResponse)
def update_anexos(
    id: int,
    anexos_req: AnexosRequest,
    db_conn: connection = Depends(get_db)
    ...
    if not anexos_db:
         raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Anexos não encontrada para atualização."
        )

...
from app.schemas.instrumentos_schema import InstrumentosRequest
...
class InstrumentosRepository:
    ...
    #Criar (C)
    def create(self, Instrumentos_req: InstrumentosRequest) -> Instrumentos:
        cursor = None
        try:
            ...
            sql = """
                ...
                VALUES (%s) 
                RETURNING * """
            cursor.execute(sql, (Instrumentos_req.nome,))
    ...
    #Listar (R)
    def get_all(self) -> list[Instrumentos]:
        cursor = None
        try:
            cursor = self.db_conn.cursor(cursor_factory=DictCursor)
    ...
    #Buscar pelo ID
    def get_by_id(self, id: int) -> Instrumentos | None:
        cursor = None
        try:
            cursor = self.db_conn.cursor(cursor_factory=DictCursor)
            ...
            if data:
                return Instrumentos(id=data['id'], nome=data['nome'])
    ...
    #Atualizar (U)
    def update(self, id: int, Instrumentos_req: InstrumentosRequest) -> Instrumentos | None:
        cursor = None
        try:
            cursor = self.db_conn.cursor(cursor_factory=DictCursor)
            ...
            sql = """
                ...
                RETURNING *
            """
            cursor.execute(sql, (Instrumentos_req.nome, id))
    ...
    def get_by_nome(self, nome: str) -> Instrumentos | None:
        cursor = None
        try:
            cursor = self.db_conn.cursor(cursor_factory=DictCursor)
    ...
    def get_or_create(self, nome: str) -> Instrumentos:
        ...
        try:
            ...
            new_data = cursor.fetchone()
            self.db_conn.commit()
            return Instrumentos(id=new_data['id'], nome=new_data['nome'])

...
from app.schemas.pedidos_schema import PedidosRequest
from .itens_repository import ItensRepository
from .aocs_repository import AocsRepository

class PedidosRepository:
    def __init__(self, db_conn: connection):
        self.db_conn = db_conn
        self.itens_repo = ItensRepository(db_conn)
        self.aocs_repo = AocsRepository(db_conn)

    #Mapear
    def _map_row_to_model(self, row: DictCursor) -> Pedidos:
    ...
    #Criar
    def create(self, pedidos_req: PedidosRequest) -> Pedidos:
        cursor = None
        try:
            #repositório
    ...
    #Buscar pelo ID
    def get_by_id(self, id: int) -> Pedidos | None:
        cursor = None
        try:
            cursor = self.db_conn.cursor(cursor_factory=DictCursor)
    ...
    #Atualizar
    def update(self, id: int, pedidos_req: PedidosRequest) -> Pedidos | None:
        cursor = None
        try:
            #repositório
            ite = self.itens_repo.get_by_descricao(pedidos_req.item_contrato_nome)

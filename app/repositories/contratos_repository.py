...
from app.schemas.contratos_schema import ContratosRequest
from .categoria_repository import CategoriaRepository
from .instrumentos_repository import InstrumentosRepository
from .modalidade_repository import ModalidadeRepository
from .numeromodalidade_repository import NumeromodalidadeRepository
from .processoslicitatorios_repository import ProcessoslicitatoriosRepository
...
class ContratosRepository:
    def __init__(self, db_conn: connection):
        ...
        self.processoslicitatorios_repo = ProcessoslicitatoriosRepository(db_conn)

    #Mapear
    def _map_row_to_model(self, row: DictCursor) -> Contratos:
    ...
    #Criar
    def create(self, contratos_req: ContratosRequest) -> Contratos:
        cursor = None
        try:
            #repositório
    ...
    #Buscar pelo ID
    def get_by_id(self, id: int) -> Contratos | None:
        cursor = None
        try:
            cursor = self.db_conn.cursor(cursor_factory=DictCursor)
    ...
    def get_by_numero_contrato(self, numero_contrato: str) -> Contratos | None:
        cursor = None
        try:
            cursor = self.db_conn.cursor(cursor_factory=DictCursor)
    ...
    #Listar
    def get_all(self, mostrar_inativos: bool = False) -> list[Contratos]:
        cursor = None
        try:
            cursor = self.db_conn.cursor(cursor_factory=DictCursor)
    ...
    #Atualizar
    def update(self, id: int, contratos_req: ContratosRequest) -> Contratos | None:
        cursor = None
        try:
            #repositório
            cat = self.categoria_repo.get_or_create(contratos_req.categoria_nome)
